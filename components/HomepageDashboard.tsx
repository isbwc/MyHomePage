...
function readGeoValue(geo: Record<string, unknown>, keys: string[]): string {
...
function formatGeoSummary(geo: Record<string, unknown>): string {
  const district = readGeoValue(geo, ['district', 'districtName']);
  const city = readGeoValue(geo, ['city', 'cityName']);
  const region = readGeoValue(geo, ['region', 'regionName', 'province', 'state']);
  const country = readGeoValue(geo, ['country', 'countryName']);
